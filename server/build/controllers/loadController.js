    ...
    projectMeasures(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let proj of projectsKeys) {
                ...
                let api_query_measures = yield cross_fetch_1.default(url).then(response => {
                    return response.json();
                });
                let response = api_query_measures["component"]["measures"];
                for (let mea of response) {
                    let idMetric = yield database_1.default.then((r) => r.query('SELECT m.idmetric FROM metrics AS m WHERE m.key = ?', mea["metric"])
                        .catch(err => {
                        console.log(err);
                    }));
                    let respMeas = new project_measure_1.Project_measure(proj["idproject"], idMetric['0']['idmetric'], mea["value"]);
                    yield database_1.default.then((r) => r.query('INSERT INTO project_measures set ?', [respMeas])
                        .catch(err => {
                        console.log(err);
                    }));
                }
